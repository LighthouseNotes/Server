...
public class ExportController : ControllerBase
{
    ...
    public async Task<ActionResult<string>> ExportContemporaneousNotes(string caseId)
    {
        ...
        foreach (Database.ContemporaneousNote contemporaneousNote in caseUser.ContemporaneousNotes)
        {
            ...
            objectPath =
                $"cases/{caseId}/{userId}/contemporaneous-notes/{_sqids.Encode(contemporaneousNote.Id)}/note.txt";
            ...
            if (htmlDoc.DocumentNode.SelectNodes("//img") != null)
                             ...
                             .Where(u => u.Attributes["src"].Value.Contains(".path/")))
                {
                    ...
                    objectPath = $"cases/{caseId}/{userId}/contemporaneous-notes/images/{HttpUtility.UrlDecode(fileName)}";
        ...
        foreach (Database.Tab tab in caseUser.Tabs)
        {
            ...
            objectPath = $"cases/{caseId}/{userId}/tabs/{_sqids.Encode(tab.Id)}/content.txt";
        ...
        foreach (Database.SharedContemporaneousNote contemporaneousNote in sCase.SharedContemporaneousNotes)
        {
            ...
            string objectPath;
            ObjectStat objectMetadata;
            Database.SharedHash? objectHashes;
            MemoryStream memoryStream = new();
            using MD5 md5 = MD5.Create();
            using SHA256 sha256 = SHA256.Create();
            byte[] md5Hash;
            ...
            if (htmlDoc.DocumentNode.SelectNodes("//img") != null)
                             ...
                             .Where(u => u.Attributes["src"].Value.Contains(".path/")))
                {
                    ...
                    // Get presigned s3 url and update image src 
                    objectPath = $"cases/{caseId}/shared/contemporaneous-notes/images/{HttpUtility.UrlDecode(fileName)}";
            ...
            model.SharedContemporaneousNotes.Add(new API.SharedContemporaneousNotesExport()
            {
                ...
                {
                    Id = _sqids.Encode(contemporaneousNote.Creator.Id), JobTitle = contemporaneousNote.Creator.JobTitle, DisplayName = contemporaneousNote.Creator.DisplayName,
                    GivenName = contemporaneousNote.Creator.GivenName, LastName = contemporaneousNote.Creator.LastName, EmailAddress = contemporaneousNote.Creator.EmailAddress,
                    ProfilePicture = contemporaneousNote.Creator.ProfilePicture,
                    Organization = new API.Organization
                        { Name = contemporaneousNote.Creator.Organization.DisplayName, DisplayName = contemporaneousNote.Creator.Organization.DisplayName },
        ...
        foreach (Database.SharedTab tab in sCase.SharedTabs)
        {
            ...
            // Create variable for object path with auth0| removed
            objectPath = $"cases/{caseId}/shared/tabs/{_sqids.Encode(tab.Id)}/content.txt";
            ...
            model.SharedTabs.Add(new API.SharedTabExport()
            {
                ...
                Creator = new API.User()
                {
                    Id = _sqids.Encode(tab.Creator.Id), JobTitle = tab.Creator.JobTitle, DisplayName = tab.Creator.DisplayName,
                    GivenName = tab.Creator.GivenName, LastName = tab.Creator.LastName, EmailAddress = tab.Creator.EmailAddress,
        ...
        using StreamReader streamReader = new(@"C:\Users\ben\source\repos\LighthouseNotes\Server\MudBlazor.min.css", Encoding.UTF8);
