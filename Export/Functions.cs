...
public class Functions
{
    ...
    public Functions(SqidsEncoder<long> sqids, string caseId, string userId, Database.Case sCase, Database.CaseUser caseUser, Database.OrganizationSettings organizationSettings, TimeZoneInfo timeZone)
    {
        _sqids = sqids;
        _caseId = caseId;
        _userId = userId;
     ...
     public async Task<API.ContemporaneousNotesExport> GetContemporaneousNote( Database.ContemporaneousNote contemporaneousNote)
    {
            // Create minio client
            ...
            string objectPath = $"cases/{_caseId}/{_userId}/contemporaneous-notes/{_sqids.Encode(contemporaneousNote.Id)}/note.txt";
            ...
            if (htmlDoc.DocumentNode.SelectNodes("//img") != null)
                             ...
                             .Where(u => u.Attributes["src"].Value.Contains(".path/")))
                {
                    ...
                    string imageObjectPath = $"cases/{_caseId}/{_userId}/contemporaneous-notes/images/{HttpUtility.UrlDecode(fileName)}";
    ...
    public async Task<API.TabExport> GetTab(Database.Tab tab)
    {
        ...
        catch (ObjectNotFoundException)
        {
            ...
            {
                Content = $"Can not find the S3 object for the tab with the ID `{_sqids.Encode(tab.Id)}` at the following path `{objectPath}`.",
                Name = tab.Name
            };
        }
        ...
        if (BitConverter.ToString(md5Hash).Replace("-", "").ToLowerInvariant() != objectHashes.Md5Hash)
            return new API.TabExport()
            {
                Content = $"MD5 hash verification failed for tab with the ID `{_sqids.Encode(tab.Id)}` at the path `{objectPath}`!",
                Name = tab.Name
            };
        ...
        if (BitConverter.ToString(sha256Hash).Replace("-", "").ToLowerInvariant() != objectHashes.ShaHash)
            return new API.TabExport()
            {
                Content = $"SHA256 hash verification failed for tab with the ID `{_sqids.Encode(tab.Id)}` at the path `{objectPath}`!",
                Name = tab.Name
            };
     ...
     public async Task<API.SharedContemporaneousNotesExport> GetSharedContemporaneousNote(Database.SharedContemporaneousNote contemporaneousNote)
    {
            // Create minio client
            IMinioClient minio = new MinioClient()
                .WithEndpoint(_organizationSettings.S3Endpoint)
            ...
            // Create a variable for object path with auth0| removed 
            string objectPath = $"cases/{_caseId}/shared/contemporaneous-notes/{_sqids.Encode(contemporaneousNote.Id)}/note.txt";
            ...
            if (htmlDoc.DocumentNode.SelectNodes("//img") != null)
                             ...
                             .Where(u => u.Attributes["src"].Value.Contains(".path/")))
                {
                    // Create variable with file name
                    string fileName = img.Attributes["src"].Value.Replace(".path/", "");
                    string imageObjectPath = $"cases/{_caseId}/shared/contemporaneous-notes/images/{HttpUtility.UrlDecode(fileName)}";
